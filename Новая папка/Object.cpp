...
void Object::updateVelocity(float x, float y )
{
	...
	Velocity()->setX( ( x - prevPos()->getX() ) * (TimerIns::Instance()->DeltaTime() / 1000.0f) );
	Velocity()->setY( ( y - prevPos()->getY() ) * (TimerIns::Instance()->DeltaTime() / 1000.0f) );
...
void Object::updateAccel(float x, float y )
{
	...
	Acceleration()->setX( ( Velocity()->getX() - prevVel()->getX() ) * (TimerIns::Instance()->DeltaTime() / 1000.0f ) );
	Acceleration()->setY( ( Velocity()->getY() - prevVel()->getY() ) * (TimerIns::Instance()->DeltaTime() / 1000.0f ) );
...
float Object::getSimTimeRem()
{
	return phSimTimeRemein;
}
...
void Object::setSimTimeRem(float t)
{
	phSimTimeRemein = t;
}
